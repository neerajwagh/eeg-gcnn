...
def collect_metrics(y_probs_test, y_true_test, y_pred_test, sample_indices_test,
	...
	# pos_label="healthy"
	fpr, tpr, thresholds = roc_curve(y_true_test, y_probs_test[:,1])
...
if __name__ == "__main__":
	...
	# NOTE: splitting subjects into train+validation and heldout test sets
	...
	for FOLD_IDX in range(10):
		...
		model = EEGConvNet(reduced_sensors=REDUCED_SENSORS)
		...
		NUM_WORKERS = 6
		PIN_MEMORY = True
		heldout_test_dataset = EEGDataset(X=X, y=y, indices=heldout_test_indices, loader_type="heldout_test", 
		...
		with torch.no_grad():
			...
			for i, sample in enumerate(heldout_test_loader):
				X_batch = sample["psd_features"].to(DEVICE, non_blocking=True)
				y_batch = sample["labels"].to(DEVICE, non_blocking=True)
				window_indices += sample["dataset_idx"].numpy().tolist()
